...
void sortNamaDescending() {
    for (int i = 0; i < jumlahRuangan - 1; i++) {
        for (int j = i + 1; j < jumlahRuangan; j++) {
            if (daftarRuangan[i].nama < daftarRuangan[j].nama) {
                swap(daftarRuangan[i], daftarRuangan[j]);
            }
        }
    }
    cout << "\n>> Data ruangan telah diurutkan berdasarkan nama (Z ke A).\n";
}

void sortHargaAscending() {
    for (int i = 0; i < jumlahRuangan - 1; i++) {
        for (int j = i + 1; j < jumlahRuangan; j++) {
            if (daftarRuangan[i].harga > daftarRuangan[j].harga) {
                swap(daftarRuangan[i], daftarRuangan[j]);
            }
        }
    }
    cout << "\n>> Data ruangan telah diurutkan berdasarkan harga (kecil ke besar).\n";
}

void sortKapasitasDescending() {
    for (int i = 0; i < jumlahRuangan - 1; i++) {
        for (int j = i + 1; j < jumlahRuangan; j++) {
            if (daftarRuangan[i].kapasitas < daftarRuangan[j].kapasitas) {
                swap(daftarRuangan[i], daftarRuangan[j]);
            }
        }
    }
    cout << "\n>> Data ruangan telah diurutkan berdasarkan kapasitas (besar ke kecil).\n";
}
...
int main() {
    ...
    do {
        ...
        if (isAdmin) {
            if (pilihan == 1) {
                ...
                cout << "1. Urutkan Nama (Z ke A)\n";
                cout << "2. Urutkan Harga (kecil ke besar)\n";
                cout << "3. Urutkan Kapasitas (besar ke kecil)\n";
                cout << "Pilihan: ";
                cin >> pilihanSort;
                cin.ignore();

                if (pilihanSort == 1) sortNamaDescending();
                else if (pilihanSort == 2) sortHargaAscending();
                else if (pilihanSort == 3) sortKapasitasDescending();
            ...
            if (pilihan == 1) {
                ...
                if (count == 0) {
                    ...
                    if (pilih >= 1 && pilih <= jumlahRuangan && !daftarRuangan[pilih - 1].tersedia) {
                        ...
                        cout << "Pembayaran untuk ruangan " << daftarRuangan[pilih - 1].nama;
    ...
    } while ((isAdmin && pilihan != 6) || (!isAdmin && pilihan != 4));
